						    ...
						    <div class="tab-content">
						        ...
						        <div role="tabpanel" class="tab-pane {{ (Request::get('tab') == 'information' || Request::get('tab') == '') ? 'active' : '' }}" id="information">
						        	...
						        	<div class="row">
										...
										<div class="col-md-6">
		                                    <div class="col-group">
		                                        <label for="" class="font-weight-bold">Referral Code :</label>
		                                        @php
		                                            $rCode = $user->referral_code ?? null;
		                                            if (empty($rCode) && \Illuminate\Support\Facades\Schema::hasTable('referral')) {
		                                                $rCode = \DB::table('referral')->where('user_id', $user->id)->value('referral_code');
		                                            }
		                                            if (empty($rCode)) {
		                                                $rCode = 'FIIN' . str_pad((string)$user->id, 6, '0', STR_PAD_LEFT);
		                                            }
		                                        @endphp
		                                        <span class="badge badge-info px-2 py-1" style="font-size:14px;"><i class="fa fa-gift mr-1"></i>{{ $rCode }}</span>
		                                   </div>
		                                </div>
										...
										<div class="col-md-6">
		                                    <div class="col-group-btn">
		                                        @if ($user->statut=="no")
		                                            <a href="{{route('users.changeStatus', ['id' => $user->id])}}" class="btn btn-success btn-sm direct-click-btn" data-toggle="tooltip" data-original-title="Activate">{{trans('lang.enable_account')}}<i class="fa fa-check"></i> </a>
		                                        @else
		                                        <a href="{{route('users.changeStatus', ['id' => $user->id])}}" class="btn btn-warning btn-sm direct-click-btn" data-toggle="tooltip" data-original-title="Activate"> Disable account <i class="fa fa-check"></i> </a>
		                                        @endif
		                                   </div>
		                                </div>
