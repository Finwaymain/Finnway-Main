                ...
                database.collection('driver_location_update').get().then(async function (snapshots) {
                    ...
                    if (snapshots.docs.length > 0) {
                        ...
                        snapshots.docs.forEach((doc) => {
                            ...
                            if (data.driver_id && data.driver_id != undefined) {
                    ...
                    if (drivers.length > 0) {
                        ...
                        $.ajax({
                            ...
                            headers: {
                                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') // Fetch CSRF token dynamically
                            },

                            data: {'drivers': JSON.stringify(drivers), _token: '{{csrf_token()}}' },
