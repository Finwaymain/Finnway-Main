...
// Live Counts
$driversOnline = DB::table('tj_conducteur')->where('online', 'yes')->count();
$usersActive = DB::table('tj_user_app')->where('statut', 'yes')->count();
$totalDisputes = \Illuminate\Support\Facades\Schema::hasTable('tj_reclamation') ? DB::table('tj_reclamation')->where('statut', '!=', 'traitee')->count() : 0;
                    ...
                    <div class="premium-card" style="padding-bottom: 12px;">
                        ...
                        <div class="row">
                            <div class="col-6 text-center mb-3">
                                ...
                                <div style="font-size: 16px; font-weight: 800; color: #0F172A; margin-top: 4px;">{{ $currency_symbol ?? \App\Helpers\Helper::getCurrencySymbol() }}{{ number_format($totalCommission, 0) }}</div>
                            </div>
                            <div class="col-6 text-center mb-3">
                                <div style="font-size: 11px; font-weight: 700; color: #5B4FE9;">Total Cashback</div>
                                <div style="font-size: 16px; font-weight: 800; color: #5B4FE9; margin-top: 4px;">{{ $currency_symbol ?? \App\Helpers\Helper::getCurrencySymbol() }}{{ number_format(abs($totalCashback), 0) }}</div>
                            </div>
                            <div class="col-6 text-center mb-3">
                                <div style="font-size: 11px; font-weight: 700; color: #10B981;">Total Payout</div>
                                <div style="font-size: 16px; font-weight: 800; color: #10B981; margin-top: 4px;">{{ $currency_symbol ?? \App\Helpers\Helper::getCurrencySymbol() }}{{ number_format($totalPayout, 0) }}</div>
                            </div>
                            <div class="col-6 text-center mb-3">
                                <div style="font-size: 11px; font-weight: 700; color: #EF4444;">Total Refunds</div>
                                <div style="font-size: 16px; font-weight: 800; color: #EF4444; margin-top: 4px;">{{ $currency_symbol ?? \App\Helpers\Helper::getCurrencySymbol() }}{{ number_format(abs($totalRefunds), 0) }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                                ...
                                <tbody>
                                    @if(count($recentTransactions) > 0)
                                        @foreach($recentTransactions as $txn)
                                            <tr>
                                                <td>#{{ \App\Helpers\Helper::formatTxnId($txn->txn_id ?: $txn->id) }}</td>
                                                <td>{{ $txn->prenom }} {{ $txn->nom }}</td>
                                                <td><strong>{{ $currency_symbol ?? \App\Helpers\Helper::getCurrencySymbol() }}{{ number_format(floatval($txn->amount), 2) }}</strong></td>
                                                <td>
            ...
            <div class="premium-card">
                ...
                <div class="sidebar-list">
                    <div class="sidebar-item">
                        <span class="sidebar-label"><i class="mdi mdi-circle text-success" style="font-size: 10px;"></i> Users Active</span>
                        <span class="sidebar-value">{{ number_format($driversOnline + $usersActive) }}</span>
                    </div>
                    <div class="sidebar-item">
                    ...
                    <div class="sidebar-item">
                        ...
                        <span class="sidebar-value">{{ number_format($totalDisputes) }}</span>
                    </div>
                </div>
            </div>
