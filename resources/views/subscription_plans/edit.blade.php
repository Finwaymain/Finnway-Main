    ...
    <div class="container-fluid pt-3">
        ...
        <form action="{{ route('subscription-plans.update', $subscriptionPlan->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-4 mb-3 plan_price_div {{ old('planType', $subscriptionPlan->type) == 'paid' ? '' : 'd-none' }}">
                            ...
                            <div class="input-group">
                                <div class="input-group-prepend"><span class="input-group-text">{{ $currency_symbol ?? \App\Helpers\Helper::getCurrencySymbol() }}</span></div>
            ...
            @php
                ...
                $rawCategories = old('categories', $subscriptionPlan->business_categories ?? ($subscriptionPlan->service_permissions['categories'] ?? $subscriptionPlan->service_permissions ?? null));
                if (is_string($rawCategories)) {
                    $savedCategories = json_decode($rawCategories, true);
                } elseif (is_array($rawCategories)) {
                    $savedCategories = $rawCategories;
                } else {
                    $savedCategories = null;
                }

                $rawFreeOrders = old('category_free_orders', $subscriptionPlan->category_free_orders ?? ($subscriptionPlan->service_permissions['category_free_orders'] ?? null));
                if (is_string($rawFreeOrders)) {
                    $savedFreeOrders = json_decode($rawFreeOrders, true) ?: [];
                } elseif (is_array($rawFreeOrders)) {
                    $savedFreeOrders = $rawFreeOrders;
                } else {
                    $savedFreeOrders = is_array($rawFreeOrders) ? $rawFreeOrders : [];
                }
            @endphp
            ...
            <div class="section-card">
                ...
                <div class="section-body">
                    ...
                    <div class="d-flex flex-wrap align-items-center mb-4" id="categoryPillContainer">
                        ...
                        @php
                            $isSelected = ($savedCategories === null) ? true : in_array($cat, $savedCategories);
                        @endphp
                        <div class="category-pill {{ $isSelected ? 'active' : '' }}" onclick="toggleCategoryPill(this, '{{ $cat }}')">
                            <span class="check-box-icon">{!! $isSelected ? '<i class="fa fa-check"></i>' : '' !!}</span>
                            <input type="checkbox" name="categories[]" value="{{ $cat }}" {{ $isSelected ? 'checked' : '' }} class="category-checkbox" style="display:none;">
                            <span>{{ $cat }}</span>
                        </div>
                        @endforeach
                    </div>
                    ...
                    <div class="border-top pt-3">
                        ...
                        <div class="row" id="freeOrdersContainer">
                            ...
                            @php
                                $isSelected = ($savedCategories === null) ? true : in_array($cat, $savedCategories);
                                $catFreeOrderVal = $savedFreeOrders[$cat] ?? ($subscriptionPlan->free_ride_limit ?? 150);
                            @endphp
                            <div class="col-md-6 col-lg-4 mb-2 category-free-order-item" id="free_order_item_{{ Str::slug($cat) }}" style="{{ $isSelected ? '' : 'display:none;' }}">
                                <div class="category-free-order-row">
                                    ...
                                    <div class="input-group input-group-sm" style="width:130px;">
                                        <input type="number" name="category_free_orders[{{ $cat }}]" class="form-control form-control-custom text-center font-weight-bold" min="0" value="{{ $catFreeOrderVal }}" placeholder="Orders">
                    ...
                    <div class="section-card h-100">
                        ...
                        <div class="section-body">
                            ...
                            <div class="p-3 bg-light rounded mt-4">
                                <p class="small text-muted mb-0"><i class="fa fa-info-circle text-primary mr-1"></i>Providers subscribing to this plan will be eligible for up to {{ $currency_symbol ?? \App\Helpers\Helper::getCurrencySymbol() }}5,00,000 interest-free business loan access.</p>
                            </div>
                    ...
                    <div id="points_container">
                        @php
                            $pts = is_array($subscriptionPlan->plan_points) ? $subscriptionPlan->plan_points : (json_decode($subscriptionPlan->plan_points, true) ?? [
                                ...
                                'Interest-Free Loan Eligibility (Up to {{ $currency_symbol ?? \App\Helpers\Helper::getCurrencySymbol() }}5 Lakh)',
                                ...
                                <div class="row">
                                    <div class="col-6">
                                        ...
                                        <select name="sender_cashback_type" id="sender_cashback_type" class="form-control form-control-custom" onchange="updateCashbackLabels()">
                                            ...
                                            <option value="flat" {{ old('sender_cashback_type', $subscriptionPlan->sender_cashback_type ?? '') === 'flat' ? 'selected' : '' }}>Flat ({{ $currency_symbol ?? \App\Helpers\Helper::getCurrencySymbol() }})</option>
                                        </select>
                                    </div>
                                    <div class="col-6">
                                ...
                                <div class="row">
                                    <div class="col-6">
                                        ...
                                        <select name="receiver_cashback_type" id="receiver_cashback_type" class="form-control form-control-custom" onchange="updateCashbackLabels()">
                                            ...
                                            <option value="flat" {{ old('receiver_cashback_type', $subscriptionPlan->receiver_cashback_type ?? '') === 'flat' ? 'selected' : '' }}>Flat ({{ $currency_symbol ?? \App\Helpers\Helper::getCurrencySymbol() }})</option>
                                        </select>
                                    </div>
                                    <div class="col-6">
                    ...
                    <div class="row mt-2">
                        <div class="col-md-6 mb-3">
                            <div class="p-3 bg-light border rounded">
                                ...
                                <label class="small text-muted mb-1">Instant wallet reward when user buys this plan ({{ $currency_symbol ?? \App\Helpers\Helper::getCurrencySymbol() }})</label>
                                <input type="number" name="cashback_on_purchase" class="form-control form-control-custom" min="0" step="0.01" value="{{ old('cashback_on_purchase', $subscriptionPlan->cashback_on_purchase ?? 0) }}" placeholder="e.g. 100">
                            </div>
                        </div>
                    </div>
                </div>
...
function updateCashbackLabels() {
    ...
    if (senderSelect && senderLabel) {
        if (senderSelect.value === 'flat') {
            senderLabel.innerText = 'Enter Value ({{ $currency_symbol ?? \App\Helpers\Helper::getCurrencySymbol() }})';
    ...
    if (receiverSelect && receiverLabel) {
        if (receiverSelect.value === 'flat') {
            receiverLabel.innerText = 'Enter Value ({{ $currency_symbol ?? \App\Helpers\Helper::getCurrencySymbol() }})';
