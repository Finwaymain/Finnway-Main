...
<div class="fw-wallet-stats">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
        <span style="font-size:12px;font-weight:600;letter-spacing:0.5px;color:#a8c7fa;text-transform:uppercase;">Interest-Free Daily Wallet</span>
    ...
    <div style="font-size:28px;font-weight:700;color:#ffffff;margin-bottom:4px;">
        ₹{{ isset($wallet->available_limit) ? number_format($wallet->available_limit) : '50,000' }}
    </div>
    <div style="font-size:12px;color:#cbd5e1;margin-bottom:16px;">Total Available Loan Limit (0% Interest)</div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;padding-top:12px;border-top:1px solid rgba(255,255,255,0.12);">
        <div>
            ...
            <p style="font-size:14px;font-weight:600;color:#81c995;margin:2px 0 0;">
                ₹{{ isset($wallet->daily_limit) ? number_format($wallet->daily_limit) : '5,000' }}
            </p>
        </div>
        <div>
            ...
            <p style="font-size:14px;font-weight:600;color:#ffffff;margin:2px 0 0;">
                ₹{{ isset($wallet->credit_limit) ? number_format($wallet->credit_limit) : '50,000' }}
            </p>
        </div>
    </div>
</div>
...
<div class="fw-card" style="border-left:4px solid var(--accent);padding:14px;">
    <div style="display:flex;justify-content:space-between;align-items:center;">
        <div>
            <p style="font-size:11px;text-transform:uppercase;color:var(--gray3);font-weight:600;margin:0;">Today's Daily Repayment</p>
            <p style="font-size:18px;font-weight:700;color:var(--navy);margin:3px 0 0;">₹1,000</p>
            <p style="font-size:11px;color:var(--amber);margin:2px 0 0;">Due Today · Keeps daily usage limit unlocked</p>
        </div>
        <button type="button" class="fw-btn fw-btn-secondary" style="font-size:12px;padding:8px 12px;white-space:nowrap;" onclick="alert('Daily repayment successful! Today\'s usage limit confirmed.')">
            Pay Daily ₹1,000
        </button>
    </div>
</div>
