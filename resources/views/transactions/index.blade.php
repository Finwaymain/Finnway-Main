                        ...
                        <div class="table-responsive m-t-10">
                            ...
                            <table id="example24"
                                ...
                                @foreach($transaction as $data)
                                    ...
                                    <tr>
                                        <td>{{ \App\Helpers\Helper::formatTxnId($data->transaction_id) }}</td>
                                        <td>
                                        ...
                                        <td>
                                            @php
                                                ...
                                                $amtVal = floatval($data->amount);
                                                $isExpense = ($data->deduction_type == "0")
                                                    || ($amtVal < 0)
                                                    || (strpos($desc, 'medical') !== false)
                                                    || (strpos($desc, 'card purchase') !== false)
                                                    || (strpos($desc, 'plan purchase') !== false)
                                                    || (strpos($desc, 'purchase') !== false)
                                                    || (strpos($desc, 'debit') !== false)
                                                    || (strpos($desc, 'deduction') !== false)
                                                    || (strpos($desc, 'expense') !== false)
                                                    || (strpos($desc, 'withdrawal') !== false)
                                                    || (strpos($desc, 'payout') !== false)
                                                    || (strpos($desc, 'fee') !== false)
                                                    || (strpos($desc, 'paid') !== false)
                                                    || ($type === 'debit');
                                                $cleanAmount = abs($amtVal);
                                            @endphp

                                            @if($isExpense)
                                                @if($currency->symbol_at_right=="true")
                                                    <span style="color:#DC2626; font-weight:700;">(-{{number_format($cleanAmount,$currency->decimal_digit)."".$currency->symbole }})</span>
                                                @else
                                                    <span style="color:#DC2626; font-weight:700;">(-{{ $currency->symbole."".number_format($cleanAmount,$currency->decimal_digit) }})</span>
                                                @endif
                                            @else
                                                @if($currency->symbol_at_right=="true")
                                                    <span style="color:#16A34A; font-weight:700;">(+{{number_format($cleanAmount,$currency->decimal_digit)."".$currency->symbole }})</span>
                                                @else
                                                    <span style="color:#16A34A; font-weight:700;">(+{{ $currency->symbole."".number_format($cleanAmount,$currency->decimal_digit) }})</span>
                                                @endif
                                            @endif
                                        </td>
                                        <td>{{ $data->description ?: '-' }}</td>
                                        <td>{{ \App\Helpers\Helper::formatTxnId($data->txn_id ?: $data->transaction_id) }}</td>
                                        <td>
