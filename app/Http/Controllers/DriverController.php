...
class DriverController extends Controller
{
    ...
    public function statusAproval(Request $request, $id, $type)
    {
        ...
        if ($admin_documents == $driverDocumentCount && $driver->kyc_status == '1') {
    ...
    public function verifyAndEnable($id)
    {
        ...
        if ($driver) {
            if ($driver->kyc_status != '1') {
                return redirect()->back()->with('error', 'Cannot verify or enable driver. KYC verification is pending.');
            }
            $driver->statut = 'yes';
            $driver->is_verified = 1;
        ...
        if ($ischeck == "true") {
            if ($driver->kyc_status != '1') {
                return response()->json([
                    'success' => false,
                    'message' => 'Cannot activate driver. KYC verification is pending.'
                ], 400);
            }
            $driver->statut = 'yes';
        } else {
