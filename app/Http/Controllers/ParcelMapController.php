...

{
...

    {
        ...
        $rides = ParcelOrder::where('status', 'onride')->orderBy('created_at', 'desc')->get();

        $rides = $rides->toArray();

        $drivers =json_decode($request->input('drivers'), true);
                ...
                if (array_key_exists('id_conducteur', $ride)) {
                    ...
                    if (isset($drivers) && count($drivers) > 0) {
                        ...
                        foreach ($drivers as $driver) {
                            ...
                            if (isset($driver['driver_id']) && $driver['driver_id'] == $ride['id_conducteur']) {
