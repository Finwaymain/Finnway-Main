...
class UserPhoneController extends Controller
{
  ...
  public function UpdateUserAlternatePhone(Request $request)
  {
        $id_user = $request->get('id_user');
        $phone = $request->get('alternate_phone');
        $phone = str_replace("'","\'",$phone);
        $user_cat = $request->get('user_cat');
        $date_heure = date('Y-m-d H:i:s');
        $photo = '';

        if($user_cat == "user_app" || $user_cat == "customer"){
            $updatedata = DB::update('update tj_user_app set alternate_phone = ?,modifier = ? where id = ?',[$phone,$date_heure,$id_user]);
            ...
            $updatedata = DB::update('update tj_conducteur set alternate_phone = ?,modifier = ? where id = ?',[$phone,$date_heure,$id_user]);
  ...
  public function ToggleMarketplace(Request $request)
  {
        $id_user = $request->get('id_user');
        $marketplace_enabled = $request->get('marketplace_enabled'); // 1 or 0
        $user_cat = $request->get('user_cat');
        $date_heure = date('Y-m-d H:i:s');

        if($user_cat == "user_app" || $user_cat == "customer"){
            $updatedata = DB::update('update tj_user_app set marketplace_enabled = ?,modifier = ? where id = ?',[$marketplace_enabled,$date_heure,$id_user]);
            ...
            if ($updatedata >= 0) {
                ...
                $response['success']= 'success';
                $response['error']= null;
                $response['message']= 'Marketplace setting updated successfully';
                $response['data'] = $row;
            } else {
                $response['success']= 'Failed';
                $response['error']= 'Failed to update marketplace setting';
            }
        }elseif($user_cat == "driver"){
            $updatedata = DB::update('update tj_conducteur set marketplace_enabled = ?,modifier = ? where id = ?',[$marketplace_enabled,$date_heure,$id_user]);
