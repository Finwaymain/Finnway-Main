        ...
        if (!empty($id_requete) && !empty($driver_name) && !empty($id_user) && !empty($from_id)) {
            ...
            if (!empty($updatedata)) {
              ...
              values('" . $refAmount . "',1,'" . $paymethod . "','" . $referBy . "','" . $date_heure . "','" . $date_heure . "')");
                ...
                $message = array_merge($row, array("body" => $msg_, "title" => $title, "sound" => "mySound", "tag" => "ridecompleted", "statut" => "completed"));
            ...
            values('" . $title . "','" . $msg . "','yes','" . $date_heure . "','" . $date_heure . "','" . $to_id . "','" . $from_id . "','ridecompleted')");
                ...
                try {
                    ...
                    $uId = (int)($request->get('id_user') ?: ($row['id_user_app'] ?? 0));
                    $dId = (int)($request->get('id_conducteur') ?: ($row['id_conducteur'] ?? 0));

                    if ($rideAmount > 0) {
                        // 1. Reward referrer of Customer who booked the service
                        if ($uId > 0) {
                        ...
                        // 2. Reward referrer of Driver who completed the service
                        if ($dId > 0) {
                            \App\Services\ReferralRewardService::processReward(
                                $dId,
                                'driver',
                    ...
                    \Illuminate\Support\Facades\Log::error("Ride referral reward error: " . $refEx->getMessage());
                }
