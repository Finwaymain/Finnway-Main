...

{
...

    {
        ...
        if (!empty($ride_id)) {
            ...
            Requests::rotateRequestIfNeeded($ride_id);
            ...
            $row = DB::table('tj_requete')
                ...
                ->select(
                    ...
                    'tj_user_app.prenom',
                    'tj_user_app.phone',
                    'tj_user_app.photo_path as user_photo_path',
            ...
            if ($row) {
                ...
                $sql_nb_avis_driver = DB::table('tj_user_note')
                    ...
                    ->where('id_user_app', '=', $row->id_user_app)
                    ->get();
                ...
                // Enrich response with driver info when a driver is assigned
                $id_conducteur = $row->id_conducteur;
                if ($id_conducteur && $id_conducteur != '0') {
                    $driver = DB::table('tj_conducteur')
                        ->select('nom', 'prenom', 'phone', 'photo_path')
                    ...
                    if ($driver) {
                        ...
                        // If user_type parameter is 'driver', we return the passenger's photo in photo_path.
                        // Otherwise, we return the driver's photo in photo_path.
...

    {
        ...
        if ($request->has('page') || $request->has('limit')) {
            ...
            $offset = ($page - 1) * $limit;
            $sql = $sql->skip($offset)->take($limit)->get();
        } else {
            $sql = $sql->get();
        }
        ...
        foreach ($sql as $row) {
            ...
            if ($row->trajet != '') {
                ...
                $row->trajet = Helper::resolveImagePath($row->trajet, 'images/recu_trajet_course');
