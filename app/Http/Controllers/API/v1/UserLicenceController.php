...
class UserLicenceController extends Controller
{
  ...
  public function updateUserLicence(Request $request)
  {
        try {
            ...
            try {
                $filename = Helper::uploadToImageKit($image, '/driver/licence');
            } catch (\Throwable $e) {
                \Log::warning('ImageKit upload for driver licence failed, falling back to local: ' . $e->getMessage());
                $filename = 'Driver_Licence_' . time() . '_' . rand(100, 999) . '.' . $extenstion;
                if ($isImage) {
                    try {
            ...
            $updatedata = DB::update('update tj_conducteur set photo_licence = ?, photo_licence_path = ?, modifier = ? where id = ?', [$filename, $filename, $date_heure, $id_user]);
            ...
            return response()->json(['success' => 'Failed', 'error' => 'Licence Not Updated']);
        } catch (\Throwable $e) {
            \Log::error('updateUserLicence error: ' . $e->getMessage());
