...

{
...

  {
...

   {
     ...
     \Log::info('Vehicle register raw payload: ', $request->all());
     ...
     $numberplate = $request->get('numberplate') ?? $request->get('carregistration');
     ...
     // Sanitize bike passenger count
     $vehType = VehicleType::find($id_categorie_vehicle);
     $isBikeVeh = false;
     if ($vehType && (stripos($vehType->libelle, 'bike') !== false || stripos($vehType->libelle, 'scooter') !== false || stripos($vehType->libelle, 'motorcycle') !== false || stripos($vehType->libelle, '2-wheeler') !== false)) {
         $isBikeVeh = true;
     }
     if ($isBikeVeh && (empty($passenger) || (int)$passenger > 2)) {
         $passenger = '1';
     }

     if (empty($id_driver) || empty($zone_id) || empty($id_categorie_vehicle) || empty($brand) || empty($model) || empty($numberplate)) {
         return response()->json([
             'success' => 'Failed',
             'error' => 'Missing required fields. Please ensure brand, model, numberplate, vehicle category, and zone are selected.'
         ]);
     }
    ...
    $chkdriver = Driver::where('id',$id_driver)->first();
     ...
     if(!empty($chkdriver)){
       ...
       if (!empty($chkid)){ 
          ...
          $updatedata = DB::update('update tj_conducteur set statut_vehicule = ?, zone_id = ?, adminCommission = ? where id = ?', ['yes', $zone_id, $commission, $id_driver]);
...

{
  ...
  $query = VehicleType::select('tj_type_vehicule.*')
  ...
  if (!empty($categoryId)) {
      $query->whereHas('userCategories', function ($q) use ($categoryId) {
          $q->where('tj_categorie_user.id', $categoryId);
      });
  }
