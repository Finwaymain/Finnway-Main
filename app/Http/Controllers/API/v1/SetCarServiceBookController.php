...
class SetCarServiceBookController extends Controller
{
  ...
  public function updateCarServiceBook(Request $request)
  {
        ...
        } else
        {
            ...
            try {
                $filename = Helper::uploadToImageKit($file, '/driver/carservice');
            } catch (\Exception $e) {
                \Log::warning('ImageKit upload for driver car service book failed, falling back to local: ' . $e->getMessage());
                $extenstion = $file->getClientOriginalExtension();
                $filename = 'Driver_Car_Service_'.time().'.'.$extenstion;
