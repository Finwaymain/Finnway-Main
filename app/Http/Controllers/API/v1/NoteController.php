    ...
    if ($id_user_app && $id_conducteur && $note_value && $comment) {
      ...
      if ($chknote->count() > 0) {
        if (!empty($ride_type) && $ride_type == 'parcel') {
          ...
          $updatedata = DB::update('update tj_note set niveau = ?,modifier = ?,comment = ? where id_conducteur = ? AND id_user_app = ? AND ride_id = ?', [$note_value, $date_heure, $comment, $id_conducteur, $id_user_app,$ride_id]);
