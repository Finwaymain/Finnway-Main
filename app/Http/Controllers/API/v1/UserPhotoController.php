...
class UserPhotoController extends Controller
{
    ...
    public function updateUserPhoto(Request $request)
    {
        ...
        if ($user_cat == "user_app") {
            ...
            if (empty($image)) {
                ...
                try {
                    $filename = $this->uploadToImageKit($file, '/user/profile');
                } catch (\Exception $e) {
                ...
                foreach ($sql as $row)
                    if ($row) {
                        ...
                        if ($row->photo_path != '') {
                            if (filter_var($row->photo_path, FILTER_VALIDATE_URL)) {
                                $image = $row->photo_path;
                            } elseif (file_exists(public_path('assets/images/users' . '/' . $row->photo_path))) {
            ...
            if (empty($image)) {
                ...
                $file = $request->file('image');
                try {
                    $filename = $this->uploadToImageKit($file, '/driver/profile');
                } catch (\Exception $e) {
                ...
                foreach ($sql as $row)
                    if ($row) {
                        ...
                        if ($row->photo_path != '') {
                            if (filter_var($row->photo_path, FILTER_VALIDATE_URL)) {
                                $image = $row->photo_path;
                            } elseif (file_exists(public_path('assets/images/driver' . '/' . $row->photo_path))) {
    ...
    private function uploadToImageKit($file, $folder = '/user/profile')
    {
