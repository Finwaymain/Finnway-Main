...
class RequeteController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        if (!empty($id_driver)) {
            ...
            $driverData = DB::table('tj_conducteur')
                ->leftJoin('tj_vehicule', 'tj_vehicule.id_conducteur', '=', 'tj_conducteur.id')
                ->select('tj_conducteur.latitude', 'tj_conducteur.longitude', 'tj_vehicule.id_type_vehicule')
                ->where('tj_conducteur.id', $id_driver)
                ->first();

            $lat = $driverData->latitude ?? 0;
            $lng = $driverData->longitude ?? 0;
            ...
            $sql = DB::table('tj_requete')
                ...
                ->select(
                    ...
                    'tj_requete.rejected_driver_id',
                    DB::raw("3959 * acos(cos(radians(" . floatval($lat) . "))
                        * cos(radians(tj_requete.latitude_depart))
                        * cos(radians(tj_requete.longitude_depart) - radians(" . floatval($lng) . "))
                        + sin(radians(" . floatval($lat) . "))
                        * sin(radians(tj_requete.latitude_depart))) AS driver_distance")
                )
                ->where('tj_requete.id_payment_method', '=', DB::raw('tj_payment_method.id'))
                ->where(function ($query) use ($id_driver, $driverData) {
                    $query->where('tj_requete.id_conducteur', '=', $id_driver)
                          ->orWhere(function ($q) use ($driverData) {
                              $q->where('tj_requete.id_conducteur', '=', '0')
                                ->where('tj_requete.statut', '=', 'new');
                              if ($driverData) {
                                  $q->where('tj_requete.id_type_vehicule', '=', $driverData->id_type_vehicule);
                              }
                          });
                })
            ...
            foreach ($sql as $row) {
                if ($row->id_conducteur == '0') {
                    if ($row->driver_distance > $radius) {
                        continue;
                    }
                    $rejDriverIds = array();
                    if (!empty($row->rejected_driver_id)) {
                        $rejDriverIds = json_decode($row->rejected_driver_id, true);
                    }
