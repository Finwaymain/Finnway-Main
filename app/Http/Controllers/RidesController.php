...

{
...

    {
        ...
        if ($id != '' || $id != null) {
            $sql->where('tj_requete.id_conducteur', '=', $id);
        }
        $totalRides = $sql->count();
        $totalNewRides = (clone $sql)->where('tj_requete.statut', 'new')->count();
        $totalOnRides = (clone $sql)->where('tj_requete.statut', 'on ride')->count();
        $totalCompletedRides = (clone $sql)->where('tj_requete.statut', 'completed')->count();
        if ($request->has('datepicker_from') && $request->datepicker_from != '' && $request->has('datepicker_to') && $request->datepicker_to != '') {
            ...
            $sql->whereDate('tj_requete.creer', '>=', $fromDate)
            ...
            $toDate = $request->input('datepicker_to');
            $sql->whereDate('tj_requete.creer', '<=', $toDate);
            ...
            if ($search == "dispatcher") {
                $sql->where('tj_requete.ride_type', 'dispatcher');
            } elseif ($search == "driver_created") {
                $sql->where('tj_requete.ride_type', 'driver');
            } else {
                $sql->where('tj_requete.ride_type', NULL);
            }
        }
        if ($request->filled('daterange')) {
            ...
            $sql->whereBetween('tj_requete.creer', [$startDate, $endDate]);
        }
        if ($request->has('status_selector') && $request->status_selector != '') {
            $status = $request->input('status_selector');
            $sql->where('tj_requete.statut', 'LIKE', '%' . $status . '%');
        }
        $rides = $sql->orderBy('tj_requete.id', 'desc')->paginate(20);

        return view("rides.all", compact('rides', 'currency', 'totalRides', 'id', 'totalNewRides', 'totalOnRides', 'totalCompletedRides'));
    }
